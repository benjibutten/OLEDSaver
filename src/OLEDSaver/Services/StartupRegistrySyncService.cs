...
/// <summary>
/// Mirrors the "Start with Windows" setting into HKCU\...\Run. Per-user, so it
/// needs no elevation, and the startup command carries <c>--minimized</c> so the
/// app comes up in the tray instead of opening a window at every logon.
/// </summary>
public sealed class StartupRegistrySyncService
{
    ...
    public const string AppRegistryName = "OLEDSaver";
    ...
    public static string BuildStartupCommand(string exePath) => $"\"{exePath}\" --minimized";
    ...
    private sealed class RegistryStartupStore : IStartupRegistryStore
    {
        ...
        public void SetValue(string name, string value) => _key.SetValue(name, value);
