...
{
    private const string LatestReleaseUrl = "https://api.github.com/repos/benjibutten/OLEDSaver/releases/latest";
    ...
    public GitHubUpdateService(HttpClient? httpClient = null, string? statePath = null)
    {
        ...
        _httpClient.DefaultRequestHeaders.UserAgent.ParseAdd("OLEDSaver-Updater/1.0");
    ...
    public async Task<UpdateInfo?> CheckAsync(Version currentVersion, bool force, CancellationToken cancellationToken = default)
    {
        ...
        string pageUrl = root.GetProperty("html_url").GetString()
            ?? "https://github.com/benjibutten/OLEDSaver/releases/latest";
        return new UpdateInfo(releaseVersion!, tagName, zipUri, checksumUri, new Uri(pageUrl));
    }

    /// <param name="restartMinimized">
    /// Whether the app should go back to the tray after the update rather than opening
    /// its window; see <see cref="UpdateInstaller.RestartMinimizedArgument"/>.
    /// </param>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            progress?.Report(new UpdateProgress("Downloading the update…", 0));
            await DownloadFileAsync(update.DownloadUri, zipPath, progress, cancellationToken);

            progress?.Report(new UpdateProgress("Verifying the download…"));
            ...
            if (restartMinimized)
                startInfo.ArgumentList.Add(UpdateInstaller.RestartMinimizedArgument);

            progress?.Report(new UpdateProgress(
                requiresElevation ? "Waiting for approval from Windows…" : "Starting the installation…"));
        ...
        while ((bytesRead = await source.ReadAsync(buffer, cancellationToken).ConfigureAwait(false)) != 0)
        {
            ...
            if (totalBytes > 0)
            {
                ...
                if (percentage != lastReportedPercentage)
                {
                    ...
                    progress?.Report(new UpdateProgress("Downloading the update…", percentage));
                }
            }
        }
    }
